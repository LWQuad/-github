...
BOOL CREATE_FONTHANDLE::CREATE_FONT_NONEDGE(int fontsize, const char* fontpath, const char* fontname)
{
	strcpy_s(this->path, sizeof(this->path), fontpath);	//パスをコピー
	strcpy_s(this->name, sizeof(this->name), fontname);	//フォント名をコピー
	this->handle = -1;								//ハンドルを初期化
	this->size = fontsize;								//サイズを引数にする
	this->bold = 1;								//太さ1
	this->type = DX_FONTTYPE_ANTIALIASING_4X4;	//アンチエイリアシング付きのフォント
